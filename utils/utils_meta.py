...
class Train_Rec_Reg_Model():
    ...
    def __init__(
        ...
        self.val_dist_min_R_R = val_loss_min
        self.val_dist_min_R_T = val_loss_min
        self.val_dist_min_T_R = val_loss_min
        self.val_dist_min_T_T = val_loss_min
        ...
        # set optimiser
        self.optimiser_rec = torch.optim.Adam(self.model.parameters(), lr=self.opt.LEARNING_RATE_rec)
        self.optimiser_reg = torch.optim.Adam(self.VoxelMorph_net.parameters(), lr=self.opt.LEARNING_RATE_reg)
    ...
    def train_rec_model(self):
        ...
        # training: 
        # using reconstruction loss and regisatrtion loss to update reconstruction network, using train dataset
        # using registartion loss to update regisatrtion network, using valdation network
        # val: 
        # using val set to evaluate reconstruction network
        # using train set to evaluate regisatrtion network

        for epoch in range(int(self.opt.retain_epoch), int(self.opt.retain_epoch)+self.opt.NUM_EPOCHS):
            # 记录epoch开始时间
            epoch_start_time = time.time()
            print(f"\n=== Meta Epoch {epoch} Started ===")
            ...
            train_epoch_loss_reg_only = 0
            train_epoch_wrap_dist_reg = 0
            train_epoch_dist_reg = 0

            train_loder_reg = iter(self.val_loader_rec)
            val_loder_reg = iter(self.train_loader_rec)
            ...
            for step, (frames, tforms, tforms_inv) in enumerate(self.train_loader_rec):
                ...
                # meta learning
                # get a random minibatch from the validation with replacement
                try:
                    frames_reg_train, tforms_reg_train, tforms_inv_reg_train = next(train_loder_reg)
                except:
                    print('initialise iterator again')
                    train_loder_reg = iter(self.val_loader_rec)
                    frames_reg_train, tforms_reg_train, tforms_inv_reg_train = next(train_loder_reg)
                # updata registration
                ...
                loss_reg_train,wrap_dist_reg_train,dist_reg = self.train_reg_model(frames_reg_train, tforms_reg_train, tforms_inv_reg_train,'train',step,epoch)
                ...
                if self.opt.Loss_type == "MSE_points":    
                    ...
                    if self.opt.Loss_type == "reg":
                        ...
                        loss = loss1 + wrap_mseloss + self.regularization(ddf)
                ...
                train_epoch_loss_reg_only += loss_reg_train.item()
                train_epoch_wrap_dist_reg += wrap_dist_reg_train.item()
                train_epoch_dist_reg += dist_reg.item()
            ...
            train_epoch_loss_reg_only /= (step + 1)
            train_epoch_wrap_dist_reg /= (step + 1)
            train_epoch_dist_reg /= (step + 1)

            if epoch in range(0, self.opt.NUM_EPOCHS, self.opt.FREQ_INFO):
                print('[Rec - Epoch %d] train-loss=%.3f, train-dist=%.3f, train-reg-only=%.6f' % (epoch, train_epoch_loss, train_epoch_dist,train_epoch_loss_reg_only))
            ...
            if epoch in range(0, self.opt.NUM_EPOCHS, self.opt.val_fre):
                ...
                epoch_loss_val = 0
                epoch_dist_val,epoch_wrap_dist_val = 0,0
                epoch_loss_val_reg = 0
                epoch_loss_val_rec = 0

                epoch_loss_val_reg_only = 0
                epoch_wrap_dist_reg_val = 0
                epoch_dist_reg_val = 0
                ...
                for step, (fr_val, tf_val, tf_val_inv) in enumerate(self.val_loader_rec):
                    ...
                    if self.opt.Loss_type == "MSE_points":
                        ...
                        if self.opt.Loss_type == "reg":
                            ...
                            loss_val = loss1_val + wrap_mseloss_val + self.regularization(ddf_val)
                    ...
                    # validate reg network
                    try:
                        frames_reg_val, tforms_reg_val, tforms_inv_reg_val = next(val_loder_reg)
                    except:
                        print('initialise iterator again')
                        val_loder_reg = iter(self.train_loader_rec)
                        frames_reg_val, tforms_reg_val, tforms_inv_reg_val = next(val_loder_reg)
                    
                    loss_reg_val,wrap_dist_reg_val,dist_reg_val = self.train_reg_model(frames_reg_val, tforms_reg_val, tforms_inv_reg_val,'val',step,epoch)
                    ...
                    epoch_loss_val_reg_only = epoch_loss_val_reg_only + loss_reg_val.item()
                    epoch_wrap_dist_reg_val = epoch_wrap_dist_reg_val + wrap_dist_reg_val.item()
                    epoch_dist_reg_val += dist_reg_val.item()
                ...
                epoch_loss_val_reg_only /= (step+1)
                epoch_wrap_dist_reg_val /= (step+1)
                epoch_dist_reg_val /= (step+1)

                # save models every 100 epoches
                self.save_rec_model(epoch)
                self.save_reg_model(epoch)


                self.save_best_models_val_T_T(epoch,epoch_dist_val)
                self.save_best_models_val_T_R(epoch,epoch_wrap_dist_val)

                self.save_best_models_val_R_T(epoch,epoch_dist_reg_val)
                self.save_best_models_val_R_R(epoch,epoch_wrap_dist_reg_val)
               


                if epoch in range(0, self.opt.NUM_EPOCHS, self.opt.FREQ_INFO):
                    print('[Rec - Epoch %d] val-loss=%.3f, val-dist=%.3f, val-reg-only=%.6f' % (epoch, epoch_loss_val, epoch_dist_val,epoch_loss_val_reg_only))
                ...
                loss_reg = {'train_epoch_loss_reg_only': train_epoch_loss_reg_only,            
                            'epoch_loss_val_reg_only':epoch_loss_val_reg_only,
                            }
                ...
                dist_wrap_reg_only = {'train_wrap_dist': train_epoch_wrap_dist_reg, 
                            'val_wrap_dist':epoch_wrap_dist_reg_val,
                            }
                
                dist_reg = {'train_dist_reg_T': train_epoch_dist_reg, 
                            'val_dist_reg_T':epoch_dist_reg_val
                            }

                add_scalars_reg(self.writer, epoch, loss_reg,'reg')

                add_scalars_wrap_dist(self.writer, epoch, dist_wrap,'rec')
                add_scalars_wrap_dist(self.writer, epoch, dist_wrap_reg_only,'reg')
                add_scalars_reg_T(self.writer, epoch, dist_reg,'T_dist_in_reg')
            ...
            print(f"=== Meta Epoch {epoch} Completed in {epoch_duration_str} ===")
    ...
    def train_reg_model(self,frames, tforms, tforms_inv, train_val,step,epoch):
        ...
        if self.opt.ddf_dirc == 'Move' and self.opt.Conv_Coords == 'optimised_coord':
            # the function below is suit for points at optimised coodinates system
            # wrap_dist is the MSE between wrapped prediction and ground truth
            mseloss, wrap_dist,pred_pts_warped = wrapped_pred_dist(ddf,pred_pts,labels,self.option,frames.shape[2],frames.shape[3],convR_batched,minxyz_all,self.device)
        ...
        if self.opt.Loss_type == "rec_reg":
            loss = loss2
        elif self.opt.Loss_type == "wraped" and self.opt.ddf_dirc == 'Move':
            loss = mseloss + self.regularization(ddf)
    ...
    def load_rec_model_epoch(self,epoch):
        # load model at specific epoch
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_epoch'+str(epoch)),map_location=torch.device(self.device)))
        except:
            raise('No best model saved at the moment...')
        
    def load_reg_model_epoch(self,epoch):
        # load model at specific epoch
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_reg_epoch'+str(epoch)),map_location=torch.device(self.device)))
        except:
            raise('No best model saved at the moment...')
    ...
    def save_best_models_val_T_T(self,epoch,running_dist):
        if running_dist < self.val_dist_min_T_T:
            self.val_dist_min_T_T = running_dist
            file_name = os.path.join(self.opt.SAVE_PATH, 'config.txt')
            with open(file_name, 'a') as opt_file:
                opt_file.write('------------ best val dist T T - epoch %s: dist = %f -------------\n' % (str(epoch),running_dist))
            if self.opt.multi_gpu:
                torch.save(self.model.module.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_T_T_T'))
                torch.save(self.VoxelMorph_net.module.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_T_T_R'))

            else:
                torch.save(self.model.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_T_T_T' ))
                torch.save(self.VoxelMorph_net.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_T_T_R' ))
    ...
    def save_best_models_val_T_R(self, epoch,running_dist):
        if running_dist < self.val_dist_min_T_R:
            self.val_dist_min_T_R = running_dist
            file_name = os.path.join(self.opt.SAVE_PATH, 'config.txt')
            with open(file_name, 'a') as opt_file:
                opt_file.write('------------ best val dist T R - epoch %s: dist = %f -------------\n' % (str(epoch),running_dist))
            if self.opt.multi_gpu:
                torch.save(self.model.module.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_T_R_T'))
                torch.save(self.VoxelMorph_net.module.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_T_R_R'))
    ...
    def save_best_models_val_R_T(self,epoch,running_dist):
        if running_dist < self.val_dist_min_R_T:
            self.val_dist_min_R_T = running_dist
            file_name = os.path.join(self.opt.SAVE_PATH, 'config.txt')
            with open(file_name, 'a') as opt_file:
                opt_file.write('------------ best val dist R T - epoch %s: dist = %f -------------\n' % (str(epoch),running_dist))
            if self.opt.multi_gpu:
                torch.save(self.model.module.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_R_T_T'))
                torch.save(self.VoxelMorph_net.module.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_R_T_R'))

            else:
                torch.save(self.model.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_R_T_T' ))
                torch.save(self.VoxelMorph_net.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_R_T_R' ))

            print('Best val dist R_T parameters saved.')


    def save_best_models_val_R_R(self,epoch,running_dist):
        if running_dist < self.val_dist_min_R_R:
            self.val_dist_min_R_R = running_dist
            file_name = os.path.join(self.opt.SAVE_PATH, 'config.txt')
            with open(file_name, 'a') as opt_file:
                opt_file.write('------------ best val dist R R - epoch %s: dist = %f -------------\n' % (str(epoch),running_dist))
            if self.opt.multi_gpu:
                torch.save(self.model.module.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_R_R_T'))
                torch.save(self.VoxelMorph_net.module.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_R_R_R'))

            else:
                torch.save(self.model.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_R_R_T' ))
                torch.save(self.VoxelMorph_net.state_dict(), os.path.join(self.opt.SAVE_PATH, 'saved_model', 'best_val_dist_R_R_R' ))

            print('Best val dist R_R parameters saved.')
